...
with st.sidebar:
    ...
    uploaded_files = st.file_uploader(
        "上传文档(支持 docx / xlsx / txt / md)",
        type=[ext.lstrip(".") for ext in SUPPORTED_EXTENSIONS],
        accept_multiple_files=True,
    )

    # "导入到知识库" 按钮
    if st.button("⬆️ 导入到知识库", disabled=not uploaded_files, use_container_width=True):
        any_success = False
        for f in uploaded_files:
            try:
                ...
                st.success(f"✅ {f.name} 导入完成({count} 块)")
                any_success = True
            except Exception as e:
                # 一个文件失败不该拖累其他文件
                st.error(f"❌ {f.name} 导入失败:{friendly_message(e)}")
        if any_success:
    ...
    status = get_status()
    st.metric("总块数", status["total_chunks"])
    ...
    if status["source_files"]:
        ...
        for filename in status["source_files"]:
            ...
            if col2.button("🗑️", key=f"del_{filename}", help=f"删除 {filename}"):
                delete_document(filename)
                st.rerun()
    else:
        st.info("知识库还是空的,先在上面上传几个文件吧。")
...
# ---------------------------------------------------------------------------
st.title("📚 企业知识库问答系统")
st.caption("基于 混合检索(Vector + BM25) + CrossEncoder 重排序 + GPT 生成")
...
def render_sources(sources: list, expanded: bool = False) -> None:
    ...
    if not sources:
        return
    with st.expander(f"📌 查看来源({len(sources)} 条)", expanded=expanded):
        for i, src in enumerate(sources, start=1):
            ...
            st.markdown(
                f"**[{i}] {filename}** "
                f"_(第 {chunk_idx + 1}/{chunk_total} 块, 重排序分: {score:.4f})_"
            )
...
if query:
    ...
    with st.chat_message("assistant"):
        ...
        if rewritten_query != query:
            st.caption(f"🔄 理解为:{rewritten_query}")
        ...
        try:
            ...
            full_answer = f"[错误] {e.user_message}"

        except Exception as e:
            # 未预期错误 —— 别崩,给个兜底提示
            answer_slot.error(friendly_message(e))
            full_answer = f"[系统错误] {type(e).__name__}"
